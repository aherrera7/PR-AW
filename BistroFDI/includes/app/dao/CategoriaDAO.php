...
class CategoriaDAO { 
    ...
    public function findAll(): array {
        ...
        if (!$rs) { throw new RuntimeException("Error BD (findAll categorias): " . $this->conn->error); }
    ...
    public function findById(int $id): ?CategoriaDTO {
        ...
        if (!$stmt->execute()) { throw new RuntimeException("Error execute (findById categorias): " . $stmt->error); }
    ...
    public function insert(string $nombre, ?string $descripcion, ?string $imagen): int {
        ...
        if (!$stmt->execute()) { throw new RuntimeException("Error execute (insert categorias): " . $stmt->error); }
    ...
    public function update(int $id, string $nombre, ?string $descripcion, ?string $imagen): bool {
        ...
        if (!$stmt->execute()) { throw new RuntimeException("Error execute (update categorias): " . $stmt->error); }
        $stmt->close();
        return true;
    }
    ...
    public function delete(int $id): bool {
        ...
        $stmt->bind_param('i', $id);
        if (!$stmt->execute()) { throw new RuntimeException("Error execute (delete categorias): " . $stmt->error); }
        $stmt->close();
        return true;
    }
}